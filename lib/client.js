...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const M15 = (() => {
      ...
      const { loadXterm } = M7
      const { attachLinkProvider, readBufferLine } = M13
      const { attachContextMenu } = M14
    /**
    ...
    M17.useState = useState;
    M17.useEffect = useEffect;
    M17.useLayoutEffect = useLayoutEffect;
    M17.useMemo = useMemo;
    M17.useReducer = useReducer;
    M17.useCallback = useCallback;
    M17.useRef = useRef;
    M17.useContext = useContext;
    M17.useSyncExternalStore = useSyncExternalStore;
    M17.Fragment = Fragment;
    M17.memo = memo;
    M17.forwardRef = forwardRef;
    M17.createContext = createContext;
    M17.createElement = createElement;
    M17.cloneElement = cloneElement;
    M17.Children = Children;
    M17.h = h;

    // ── 入口模块的契约导出（插件系统读取 apply / inject） ──
    exports.inject = M17.inject
    exports.apply = M17.apply
